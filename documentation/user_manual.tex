...
\subsubsection{Install Matching component}
...
If you would like to use Warden\footnote{\url{https://warden.cesnet.cz}} as a source of incoming events (or send predicted events into the Warden),
you have to configure your Warden sender/receiver. Then you can use prepared script \texttt{kafka\_producer.py} for reading events from a directory,
as the Warden receiver saves one event per file into a directory, and sending them into a Kafka topic.
Or you can use \texttt{kafka\_consumer.py}, which will read events from a Kafka topic and saves one event per file into a directory, as Warden sender expects.
Both scripts are in the directory \texttt{services/warden-kafka-clients}.
