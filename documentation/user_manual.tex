...
Restapi service provides REST API which is used for communications with Dashboard. API is exposed on port 7777.
This service is not mandatory, if you are not planning to use Dashboard you can skip this part.

Restapi is Django application which will run on Apache2 with WSGI.
Firstly install Apache2 with WSGI support for Python 3.
\begin{lstlisting}
$ sudo apt-get -y install apache2 libapache2-mod-wsgi-py3
\end{lstlisting}

Restapi is getting information about AIDA services via dbus systemd interface, so we will have to install dbus dependencies.
\begin{lstlisting}
$ sudo apt-get -y install dbus libdbus-glib-1-dev libdbus-1-dev python-dbus
\end{lstlisting}

Now we will place the Django application into correct directory.
\begin{lstlisting}
...
\end{lstlisting}
And replace placeholder \texttt{\{\{ database\_path \}\}} in the file with \texttt{/var/aida/rules/rule.db}

Now we create config file for Apache.
\begin{lstlisting}
...
Now we just need to restart Apache2 and check that REST API is responding on address \texttt{localhost:7777}.
\begin{lstlisting}
